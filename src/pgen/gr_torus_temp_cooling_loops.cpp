...
//set h_r_targ q_cooling  global,

__managed__ Real h_r_targ;
__managed__ Real q_cooling;
//Real h_r_targ;
//Real q_cooling;
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  //set h_r_targ, q_cooling
  h_r_targ = pin->GetOrAddReal("problem", "h_r_targ",0.1);
  q_cooling = pin->GetOrAddReal("problem", "q_cooling",1);
  ...
  if (pgen.is_vertical_field) {
    ...
    if (r >= pgen.r_edge) {
      ...
      if (in_torus) {
        ...
        aphi_tilt = (rho/pgen.rho_max)-pgen.potential_cutoff;
        aphi_tilt = fmax(aphi_tilt, 0.0);
        aphi_tilt = pow(aphi_tilt, pgen.potential_rho_pow)*scaling_param;
        
        if (pgen.potential_loops > 0) {
          Real R = r*sin_vartheta; 
          aphi_tilt *= sin(pgen.potential_loops*M_PI*(R-pgen.potential_edge)/(pgen.potential_outer_edge-pgen.potential_edge));
        }
        if (pgen.psi != 0.0) {
          Real dvarphi_dtheta = -pgen.sin_psi * sin_phi / SQR(sin_vartheta);
          Real dvarphi_dphi = sin_theta / SQR(sin_vartheta)
...
void Cooling(Mesh *pm, const Real bdt) {
  ...
  if (pmbp->prad == nullptr){
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      //Find midplane rotation frequency
      Real Rotation_Frequency = 1/(spin+pow(R,1.5));

      //Find target temperature and temperature
      Real Temp_targ = (M_PI/2)*SQR(h_r_targ*R*Rotation_Frequency);
      Real Temp = (w0_(m,IEN,k,j,i)*gm1)/w0_(m,IDN,k,j,i);

      //Find Comoving Cooling Rate
      Real CoolingRate = Rotation_Frequency*w0_(m,IEN,k,j,i)*pow(Temp/Temp_targ,q_cooling);

      if((Temp>Temp_targ)&&((2*M_PI/Rotation_Frequency)>bdt)){
