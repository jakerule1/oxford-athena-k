        ...
        if((size.h_view(m).x1min <= b_box_size) && (size.h_view(m).x2min <= b_box_size) && (size.h_view(m).x3min <= b_box_size)
          ...
          par_for("populate_vect_pot_arrays", DevExeSpace(), ks,ke+1,js,je+1,is,ie+1,
            KOKKOS_LAMBDA(int k, int j, int i){
              ...
              Real x1_at_faces[3] = {x1v, x1f, x1f};
              ...
              a1(m,k,j,i) = fmax((rho_at_faces[0]-trs.potential_cutoff),0.0)*a1_c(index).real();
              a2(m,k,j,i) = fmax((rho_at_faces[1]-trs.potential_cutoff),0.0)*a2_c(index).real();
              a3(m,k,j,i) = fmax((rho_at_faces[2]-trs.potential_cutoff),0.0)*a3_c(index).real();
...
void Cooling(Mesh *pm, const Real dt, ParameterInput *pin) {
  ...
  if (pmbp->prad == nullptr){
    ...
    if (pmbp->phydro != nullptr) {
      ...
      gamma = pmbp->pmhd->peos->eos_data.gamma;
      pfloor = pmbp->pmhd->peos->eos_data.pfloor;
      w0_ = pmbp->pmhd->w0;
      u0_ = pmbp->pmhd->u0;
    }
    auto &size = pmbp->pmb->mb_size;
    ...
    int ie = indcs.ie, je = indcs.je, ke = indcs.ke;
    int nmb = pmbp->nmb_thispack;
    par_for("User_Source_Cooling", DevExeSpace(), 0,nmb-1,ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      CoolingRate *= 1.0/7.0;
      CoolingRate *= w0_(m,IEN,k,j,i)/(Cooling_Timescale*u0);
